...
public class Wordbdao implements Worddao {
    private Connection con = null;
    private PreparedStatement psmt = null;
    private ResultSet rs = null;

    private final String WORD_INSERT = "insert into  VOCAB(" +
            "word, MEANINGA, MEANINGB, MEANINGC, SIMILAR_WORDA, SIMILAR_WORDB, SIMILAR_WORDC, day) " +
            "values (?,?,?,?,?,?,?,?)";
    private final String WORD_UPDATE = "update VOCAB " +
            "set  MEANINGA=?, MEANINGB=?, MEANINGC=?, " +
            "SIMILAR_WORDA=?, SIMILAR_WORDB=?, SIMILAR_WORDC=?, day=? " +
            "where word = ?";
    private final String WORD_DELETE = "delete VOCAB where word=?";
    private final String WORD_GET = "select * from VOCAB where word=?";
    private final String WORD_LIST = "select * from VOCAB where day=?";
    private final String WORDB_LIST_ = "select * from VOCAB";
    ...
    public int selectCount(Map<String, Object> map) {
        ...
        int totalCount = 0;
        String query = "SELECT COUNT(*) FROM vocab";
        if (map.get("searchWord") != null) {
            query += " WHERE word "
                    + " LIKE '%" + map.get("searchWord") + "%'";
        }
    ...
    public int insertMyvoc(String insertingword) {
        int working = 0;
        String query1 = "SELECT * FROM vocab where word = ?";
        String query2 = "insert into sel values (?, ?, ?, ?, ?, ?, ?, ?, 'aaa', 2)";
    ...
    //    관리자 섹션
    public List<Worddto> getWordBList() {
        System.out.println("===> JDBC로 getWordBList() 기능 처리");

        List<Worddto> WordList = new ArrayList<Worddto>();
        try {
            con = JDBCUtil.getConnection();
            psmt = con.prepareStatement(WORDB_LIST_);
            rs = psmt.executeQuery();
  ...
  //나만의 단어장에 단어를 추가 합니다.
    public int insertMyvoc(String insertingword,String id) {
        int working = 0;
        String query1 = "SELECT * FROM vocab where word = ?";
        String query2 = "insert into sel values (?, ?, ?, ?, ?, ?, ?, ?, ?, 2)";
